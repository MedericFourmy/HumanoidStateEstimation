...
+ \frac{1}{2} \bfg \delta t^2 + \frac{1}{2m} \Rot{}{}^{k-1} \sum_l \Rot{B}{L}(q^{k-1}) (\forcem{}{l}^{k-1} - \noise_f^{k-1}) \delta t^2
\\
\COMd^{k}&= \COMd^{k-1} + \bfg \delta t + \frac{1}{m} \Rot{}{}^{k-1} \sum_l \Rot{B}{L}(q^{k-1}) (\forcem{}{l}^{k-1} - \noise_f^{k-1}) \delta t 
\\
\AM_c^{k} &= \AM_c^{k-1} +  ( 
...
Based on equations \ref{eq:COMDiscrete}, we can express the relationships as measurement functions $\bfy_{FT} = h_{FT}(\bfx) + \noise_{FT}$:

\begin{small}
\begin{equation}
\begin{split}
	\frac{1}{2m}\sum_l \Rot{B}{L}^{k-1} \forcem{}{l}^{k-1}\delta t^2 =& 
	\Rot{}{}^{k-1,T}(\COM^{k} - \COM^{k-1} - \COMd^{k-1} \delta t - \frac{1}{2} \bfg \delta t^2)
	+ \frac{1}{2m}\sum_l \Rot{B}{L}^{k-1} \noise_f^{k-1}\delta t^2
	\\
	\frac{1}{m} \sum_l \Rot{B}{L}^{k-1} \forcem{}{l}^{k-1} \delta t =& \Rot{}{}^{k-1,T}(\COMd^{k} - \COMd^{k-1} - \bfg \delta t) 
	+ \frac{1}{m} \sum_l \Rot{B}{L}^{k-1} \noise_f^{k-1} \delta t	
\end{split}
\label{eq:CCdFactor}
\end{equation}
\end{small}
...
\begin{equation}
\begin{split}
	(\sum_l \Rot{B}{L}^{k-1}\torquem{}{l}^{k-1} + (\posi{B}{BL}^{k-1} - \posim{B}{BC}^{k-1} + \bias_{p}^{k-1}) \times \Rot{B}{L}^{k-1}\forcem{}{l}^{k-1}) \delta t = 
	\Rot{}{}^{k-1,T}(\AM_c^{k} - \AM_c^{k-1})
	\\
	+ (\sum_l \Rot{B}{L}^{k-1}\noise_{\tau}^{k-1} + (\posi{B}{BL}^{k-1} - \posim{B}{BC}^{k-1} + \bias_{p}^{k-1})\times \Rot{B}{L}^{k-1}\noise_f^{k-1}  + \Rot{B}{L}^{k-1}\forcem{}{l}^{k-1}  \times \noise_p^{k-1} + \noise_p \times \noise_f)\delta t
\end{split}
\label{eq:LcFactor}
\end{equation}
\end{small}
...
Then, to get the covariance matrix of $\noise_{FT}$, we have to recover the linear relation $\noise_{FT} = J^{\noise_{FT}}_{\noise_{ft}}$ from eqs \ref{eq:CCdFactor} and \ref{eq:LcFactor} with $\noise_{f} = (\noise_f, \noise_{\tau}, \noise_p)$.

Neglecting the nonlinear term $\noise_p \times \noise_f$ in eq \ref{eq:LcFactor}, we get:

\begin{equation}
	J^{\noise_{FT}}_{\noise_{ft}} =
	\begin{pmatrix}
	\frac{1}{2m}\sum_l \Rot{B}{L}^{k-1} \delta t^2 & 0 & 0
	\\
	\frac{1}{m}\sum_l \Rot{B}{L}^{k-1} \delta t & 0 & 0
	\\
	[\posi{B}{BL}^{k-1} - \posim{B}{BC}^{k-1} + \bias_{p}^{k-1}]_{\times} \Rot{B}{L}^{k-1} & \Rot{B}{L}^{k-1} & [\Rot{B}{L}^{k-1}\forcem{}{l}^{k-1}]_{\times}
	\end{pmatrix}
\end{equation}
...
\COMd^{j} \delta t 
+ \frac{1}{2}\bfg \Delta t^2 + \frac{1}{2m} \Rot{}{}^{j} \sum_l \Rot{B}{L}(q^{j}) (\forcem{}{l}^{j} - \noise_f^{j})\Delta t^2 \Big]
\\
