...
\\
&= \sum_l \Rot{}{}\Rot{B}{L}(q)(\torquem{}{l} - \noise_{\tau}) + \Rot{}{}( \posi{B}{BL}(q) - (\posim{B}{BC}(q)) -  \bias_{p} - \noise_p) \times (\Rot{B}{L}(q)(\forcem{}{l} - \noise_f))
\end{split}
\end{equation}
...
\\
\frac{d\AM_c}{dt}&= \Rot{}{} \sum_l \Rot{B}{L}(q)(\torquem{}{l} - \noise_{\tau}) + (\posi{B}{BL}(q) - (\posim{B}{BC}(q)) -  \bias_{p} - \noise_p) \times (\Rot{B}{L}(q)(\forcem{}{l} - \noise_f))
\end{split}
\label{eq:COMContinuous}
\end{equation}

Great care should be given defining a measurement model for \(\posi{B}{BC}(q)\). This data is potentially corrupted by 3 factors:
...
\begin{equation}
\posim{B}{BC} = \posi{B}{BC}(q) + \bias_{p} + \noise_p 
\end{equation}
...
\\
\velm{A}{AB} = \vel{A}{AB}(q) + \noise_v ~~~ \noise_v \sim \Gaussian{0}{Q_v}, ~ Q_v \in \mathrm{R}^{3\times 3} 
\\
\Rotm{A}{B} = \Rot{A}{B}(q) Exp(\phi) ~~~ \phi \sim \Gaussian{0}{Q_{\phi}}, ~ Q_{\phi} \in \mathrm{R}^{3\times 3}
\end{split}
\end{equation}
 
An \textbf{important} thing to take into account is the fact that some of the kinematic measurement noise can be correlated. For instance the relative position shoulder-hand is strongly correlated with the shoulder-elbow one. Some others are less obvious (hand velocity/hand position?). As for the relative poses measurements, one way to deal with these hard to model correlations is to decompose the measurements so that the noise random variables can be reasonably considered independent. One idea is to consider (if such exists) a root frame by which the considered kinematic chain is passing. For instance   
...
\\
    \Delta \dot{\COM}_{ij} &\triangleq \Rot{}{}^{i,T} (\dot{\COM}^{k} - \dot{\COM}^{i} - \bfg \Delta t_{ik})
\\
...
\begin{itemize}
    ...
    \item \( \bfI(q) \): inertia matrix of the robot expressed at the COM available at XXX frame
...
\\
\dot{\COM} &= \vel{}{} + \Rot{}{}((\angvelm{B}{B} - \bias_{\omega} - \noise_{\omega}) \times (\posim{B}{BC} -  \bias_{p} - \noise_p) + (\velm{B}{BC} - \noise_v))
\\
\AM_c &= \Rot{}{}(\bfI(q) (\angvelm{B}{B} - \bias_{\omega} - \noise_{\omega}) + \AM_q (q,\dot{q}))
\end{split}
\end{equation}
...
\begin{equation}
\bfe_{ikin}(\COM, \dot{\COM}, \AM_c, \posi{}{}, \Rot{}{}, \vel{}{}, \bias_{\omega}, \bias_p) = 
\begin{pmatrix}
\bfe_{\COM}(\COM, \posi{}{}, \Rot{}{})
\\
\bfe_{\dot{\COM}}(\dot{\COM}, \vel{}{}, \Rot{}{}, \bias_{\omega}, \bias_p)\\
\bfe_{\AM_c}(\AM_c, \Rotm{}{}, \bias_{\omega})
\end{pmatrix}
=
\begin{pmatrix}
\posim{B}{BC} - (\Rot{}{}^T(\COM - \posi{}{}) + \bias_{p}) 
\\
(\angvelm{B}{B} - \bias_{\omega}) \times (\posim{B}{BC} -  \bias_{p}) + \velm{B}{BC} - \Rot{}{}^T(\dot{\COM} - \vel{}{})
\\
\bfI(q) (\angvelm{B}{B} - \bias_{\omega}) - \Rot{}{}\AM_c
\end{pmatrix}
\end{equation}
...
Neglecting the last nonlinear term in the $\dot{\COM}$ equation, we thus have a linear relation between measurement noises and error functions. 
...
Defining $\bar{\omega} = $ and $\bar{\posi{}{}} = $, the covariance structure is:
...
\\
-[\bar{\omega}]_{\times}Q_p & Q_{pv\omega}& -Q_v
\\
0 & -Q_v & Q_v
\end{pmatrix}
\end{equation}
...
and $\bar{\angvel{}{}} = \angvelm{}{} - \bias_{\angvel{}{}}$,  $\bar{p} = \posim{B}{BC} - \bias_p \bias_{\angvel{}{}}$
